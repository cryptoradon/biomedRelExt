...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (
            tokens['input_ids'].squeeze(0),         # Input IDs
            tokens['attention_mask'].squeeze(0),    # Attention mask
            input.chemicalID,                       # Chemical ID
            input.diseaseID,                        # Disease ID
            torch.tensor(start_token_index),        # Ground truth start token index
            torch.tensor(end_token_index),          # Ground truth end token index
        )
